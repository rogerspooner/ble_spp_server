...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_WRITE_EVT) {
        unsigned long* pLong = (unsigned long*)param->write.value;
        esp_log_write(ESP_LOG_INFO, GATTS_TABLE_TAG, 
            "Client is writing bda %lx attribute handle %x value_word %lx\n",
            (unsigned long)param->write.bda, (unsigned int)param->write.handle, *pLong);
    }
